...
function OpenBossMenu(society, close, options)
	...
	ESX.TriggerServerCallback('esx_society:isBoss', function(isBoss)
		if isBoss then
			...
			local action = exports["nh-context"]:ContextMenu({

				{
					header = "Menu Patron ",
					context = 'Argent Society : $'.. socMoney
				},
				{
					header = ""..   _U('withdraw_society_money'),
					context = "Retirer Argent",
					args = {"withdraw_society_money"},
				},
		
				{
					header =' ' .. _U('deposit_society_money'),
					context = "Déposer Argent",
					args = {"deposit_money"}
				},
				{
					header =' ' .. _U('employee_management'),
					context = "Management Employé",				
					event = "nh-context:ManageMent",
					args = {society}
				
				},
				{
					header =' ' .. _U('salary_management'),
					context = "Management Versement",				
					args = {"manage_grades"}
			...
			if action == 'withdraw_society_money' then
			...
			elseif action == 'deposit_money' then
			...
			elseif action == 'manage_employees' then
				--OpenManageEmployeesMenu(society)
				TriggerEvent('nh-context:ManageMent', society)
			elseif action == 'manage_grades' then
...
RegisterNetEvent("nh-context:ManageMent", function(society)
	print(society)
	local actions = exports["nh-context"]:ContextMenu({

		{
			header = "Management Employés",
		},
		{
			header = ""..  _U('employee_list'),
			context = "Regarder Liste Employés",
			args = {"employee_list"},
		},

		{
			header =' ' .. _U('recruit'),
			context = "Recruter Un Nouveau Citoyen",
			args = {"recruit"}
		},			  
	})

	if actions == 'employee_list' then
		OpenEmployeeList(society)
	elseif actions == 'recruit' then
