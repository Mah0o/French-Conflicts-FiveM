...
function GetNearbyPlayer(solo, other) -- Sélectionner un joueur si plusieurs sont collé à vous
    ...
    if #oPlayer == 0 then
        ShowNotification("~r~Action Impossible~n~~s~Personne autour de vous..")
        return false
    end
    ...
    ShowNotification("Appuyer sur ~g~E~s~ pour valider~n~Appuyer sur ~b~A~s~ pour changer de cible~n~Appuyer sur ~r~X~s~ pour annuler")
    Citizen.Wait(100)
    local cBase = 1
    cWait = true
    while GetGameTimer() <= cTimer and not xWait do
        ...
        elseif IsDisabledControlJustPressed(0, 73) then
            ShowNotification("Vous avez ~r~annulé~s~ cette ~r~action~s~")
            break
