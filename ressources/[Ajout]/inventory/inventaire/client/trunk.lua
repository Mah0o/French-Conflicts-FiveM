...
AddEventHandler("esx_inventoryhud:openTrunkInventory",
                function(data, blackMoney, cashMoney, inventory, weapons, model)
    Inv:SetTrunkInventoryData(data, blackMoney, cashMoney, inventory, weapons,
                              model)
    Inv:OpenTrunkInventory()
end)

RegisterNetEvent("esx_inventoryhud:refreshTrunkInventory")
AddEventHandler("esx_inventoryhud:refreshTrunkInventory",
...
function Inv:SetTrunkInventoryData(data, blackMoney, cashMoney, inventory,
                                   weapons, model)
    trunkData = data
...
RegisterNUICallback("PutIntoTrunk", function(data, cb)
    ...
    if type(data.number) == "number" and math.floor(data.number) == data.number then
        ...
        TriggerServerEvent("esx_trunk:putItem", trunkData.plate, data.item.type,
                           data.item.name, count, trunkData.max,
                           trunkData.myVeh, data.item.label)
    end
...
RegisterNUICallback("TakeFromTrunk", function(data, cb)
    ...
    if type(data.number) == "number" and math.floor(data.number) == data.number then
        TriggerServerEvent("esx_trunk:getItem", trunkData.plate, data.item.type,
                           data.item.name, tonumber(data.number), trunkData.max,
                           trunkData.myVeh)
    end
