            ...
            if anim then
                if not checkanim then
                    ...
                    ESX.Streaming.RequestAnimDict('missexile3', function()
                        TaskPlayAnim(PlayerPedId(), 'missexile3',
                                     'ex03_dingy_search_case_a_michael', 8.0,
                                     -8.0, -1, 35, 0.0, false, false, false)
                    end)
                elseif checkanim then
                    if IsControlJustReleased(0, 37) and checkanim then
                        ...
                        if target and distance < 1 then
                            TriggerServerEvent('debloqueontrol',
                                               GetPlayerServerId(target))
                            pasarme = false
                        end
                    end
                end
        ...
        if Shared.IncludeWeapons and weapons ~= nil then
            for key, value in pairs(weapons) do
                ...
                local playerPed = GetPlayerPed(
                                      GetPlayerFromServerId(targetPlayer))
...
AddEventHandler("esx_inventoryhud:Inv:OpenPlayerInventory",
...
RegisterCommand('fouiller', function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        if IsEntityPlayingAnim(GetPlayerPed(closestPlayer),
                               "random@arrests@busted", "idle_c", 3) then
            ExecuteCommand('me fouille quelqu\'un')
            anim = true
            Inv:Animation()
            TriggerEvent("esx_inventoryhud:Inv:OpenPlayerInventory",
                         GetPlayerServerId(closestPlayer),
                         GetPlayerName(PlayerPedId()))
        else
            ESX.ShowNotification(
                '~r~Impossible~s~~n~L\'individu doit lever les mains.')
        end
    else
        ESX.ShowNotification(
            '~r~Impossible~s~~n~Aucune personne autour de vous.')
    end
end)
    ...
    if closestPlayer == -1 or closestDistance > 1.0 then
        ...
        if GetPlayerServerId(closestPlayer) == targetPlayer then
            if type(data.number) == "number" and math.floor(data.number) ==
                ...
                TriggerServerEvent("esx_inventoryhud:tradePlayerItem",
                                   GetPlayerServerId(PlayerId()), targetPlayer,
                                   data.item.type, data.item.name, count,
                                   data.item)
            end
            Wait(250)
