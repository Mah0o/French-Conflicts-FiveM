...
InteractDrugMenu = RageUI.CreateMenu("Laboratoire", "Intéractions disponibles")
InteractListDrugMenu = RageUI.CreateSubMenu(InteractDrugMenu, "Laboratoire", "Laboratoires disponibles")
        ...
        CreateThread(function()
            while DrugsHandler.MenuIsOpen do 
                ...
                RageUI.IsVisible(InteractDrugMenu, function()
                    ...
                    for i = 1, #DrugsHandler.LaboData do 
                        if DrugsHandler.LaboData[i].owner == identifier then 
                            ...
                            RageUI.Separator("↓ ~g~Laboratoire de "..string.lower(DrugsHandler.LaboData[i].type).." n°"..DrugsHandler.LaboData[i].id.."~s~ ↓")
                ...
                RageUI.IsVisible(InteractListDrugMenu, function()
                    RageUI.Separator("↓ ~g~Liste des laboratoire(s)~s~ ↓")
                    for i = 1, #DrugsHandler.LaboData do 
                        if DrugsHandler.LaboData[i].owner ~= identifier then 
                            DrugsHandler.LaboData[i].exiting = pCoords
                            RageUI.Button("Entrer dans le laboratoire "..DrugsHandler.LaboData[i].id, false, {RightLabel = "→"}, true, {
                                onSelected = function()
                                    ...
                                    if password == DrugsHandler.LaboData[i].password then 
                                        ...
                                        ESX.ShowNotification("~r~Mot de passe invalide.")
                                    end 
                                end
                            })
                        end
...
ComputerMenu = RageUI.CreateMenu("Ordinateur", "Intéractions disponibles")
ComputerBuySuppliesMenu = RageUI.CreateSubMenu(ComputerMenu, "Commmandes", "Matières disponibles")
ComputerUpgradeMenu = RageUI.CreateSubMenu(ComputerMenu, "Améliorations", "Améliorations disponibles")
ComputerGestionMenu = RageUI.CreateSubMenu(ComputerMenu, "Gestion", "Intéractions disponibles")
ComputerStorageMenu = RageUI.CreateSubMenu(ComputerMenu, "Stockage", "Intéractions disponibles")
ComputerDepositMenu = RageUI.CreateSubMenu(ComputerStorageMenu, "Stockage", "Objets disponibles")
ComputerWithdrawMenu = RageUI.CreateSubMenu(ComputerStorageMenu, "Stockage", "Objets disponibles")
ComputerUpgradeSubMenu = RageUI.CreateSubMenu(ComputerUpgradeMenu, "Améliorations", "Améliorations disponibles")
        ...
        CreateThread(function()
            while DrugsHandler.MenuIsOpen do 
                    ...
                    for i = 1, #DrugsHandler.LaboData do 
                        ...
                        for k,v in pairs(DrugsHandler.Supplies[labo.type]) do
                            if isLocked == false then 
                                RageUI.Button(v.name, v.desc, {RightLabel = ("Prix : ~g~%s$"):format(v.price)}, true, {
                                    onSelected = function()
                ...
                RageUI.IsVisible(ComputerUpgradeMenu, function()
                    RageUI.Separator("↓ ~g~Améliorations~s~ ↓")
                    for i = 1, #DrugsHandler.LaboData do 
                        local labo = DrugsHandler.LaboData[i]
                            ...
                            for k,v in pairs(DrugsHandler.Upgrades[labo.type][currentUsing]) do 
                                if currentUsing ~= "details" then 
                                    if int == v.value then 
                                        ...
                                        RageUI.Button(v.name, v.desc, {RightLabel = ("Prix : %s.0 ~g~$"):format(v.price)}, true, {
                                            onSelected = function()
                ...
                RageUI.IsVisible(ComputerGestionMenu, function()
                    RageUI.Separator("↓ ~o~Gestion du laboratoire~s~ ↓")
                    for i = 1, #DrugsHandler.LaboData do 
                        local data = DrugsHandler.LaboData[i]
                        RageUI.Button(("État du laboratoire ( %s )"):format(labName(data.data.interiorStatus)), false, {}, true, {})
                        RageUI.Button(("Code du laboratoire - ~r~%s~s~"):format(data.password), false, {}, true, {})
                        if data.owner == yourIdentifier then 
                            ...
                            RageUI.Button("~r~Révoquer votre laboratoire~s~", "Attention cette action est irréversible.", {RightBadge = RageUI.BadgeStyle.Alert}, true, {
                                onSelected = function()
                ...
                RageUI.IsVisible(ComputerStorageMenu, function()
                    RageUI.Separator("↓ ~o~Gestion du stockage~s~ ↓")
                ...
                RageUI.IsVisible(ComputerDepositMenu, function()
                    ...
                    for _, item in pairs(ESX.PlayerData.inventory) do
                        if item.count > 0 then
                            RageUI.Button(("%s - [~r~%s~s~]"):format(item.label, item.count), false , {RightLabel = "~b~Déposer~s~ →"}, true, {
                                onSelected = function()
                ...
                RageUI.IsVisible(ComputerWithdrawMenu, function()
                    RageUI.Separator("↓ ~y~Contenu du coffre~s~ ↓")
                    for k,v in pairs (DrugsHandler.LaboData) do 
                        for t,b in pairs (v.storage) do 
                            RageUI.Button(("%s - [~r~%s~s~]"):format(b.label, b.count), false , {RightLabel = "~b~Retirer~s~ →"}, true, {
                                onSelected = function()
