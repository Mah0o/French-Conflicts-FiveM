    ...
    while true do
        ...
        if NearBank() and not isBankOpened and NearBank() ~= "update" then
        	if not Config.okokTextUI then
            	ESX.ShowHelpNotification("Appuye ~INPUT_PICKUP~ pour accèder à ton compte ~b~bancaire")
            else
            	inRange = true
            end
        ...
        if inRange and not shown then
        	shown = true
        	exports['okokTextUI']:Open('[E] Pour accèder à ton compte bancaire', 'darkblue', 'left') 
            ...
            if IsControlJustReleased(0, 38) then
            	ESX.TriggerServerCallback("okokBanking:GetPIN", function(pin)
            		if pin then
						...
						exports['okokNotify']:Alert("BANK", "Met un pin code sur ton compte", 5000, 'info')
					end
				end)
            end
	...
	elseif data.action == "deposit" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				...
				exports['okokNotify']:Alert("BANK", "Montant invalide", 5000, 'error')
			end
		else
			exports['okokNotify']:Alert("BANK", "Montant invalide", 5000, 'error')
		end
			...
			if tonumber(data.value) > 0 then
				ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
					...
					elseif isUsed == nil then
						exports['okokNotify']:Alert("BANK", "Le IBAN n existe pas", 5000, 'error')
					end
				end, data.iban:upper())
			else
				exports['okokNotify']:Alert("BANK", "Montant invalide", 5000, 'error')
	...
	elseif data.action == "change_iban" then
		if Config.CustomIBANAllowLetters then
			...
			ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				elseif isUsed ~= nil then
					exports['okokNotify']:Alert("BANK", "La banque est déjà utilisé", 5000, 'error')
				end
			end, iban)
			...
			if tonumber(data.iban) ~= nil then
				...
				ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
					...
					if isUsed == nil then
						TriggerServerEvent('okokBanking:UpdateIbanDB', iban, Config.IBANChangeCost)
					elseif isUsed ~= nil then
						exports['okokNotify']:Alert("BANK", "Iban déjà pris", 5000, 'error')
					end
				end, iban)
			else
				exports['okokNotify']:Alert("BANK", "Tu peut juste utiliser des numéro sur ton IBAN", 5000, 'error')
			end
		end
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				...
				exports['okokNotify']:Alert("BANK", "Ton PIN dois contenir 4 chiffres", 5000, 'info')
			end
		else
			exports['okokNotify']:Alert("BANK", "Tu peut juste utiliser numero", 5000, 'info')
		end
	end
end)
