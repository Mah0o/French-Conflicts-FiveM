...
AddEventHandler("okokBanking:DepositMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous avez deposité "..amount.."$", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous n avez pas l argent", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous avez retiré "..amount.."$", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous n avez pas cet argent", 5000, 'error')
	end
end)
	...
	if xPlayer.identifier ~= targetIdentifier then
		if amount <= playerMoney then
			...
			if xTarget ~= nil then
				...
				for i=1, #xPlayers, 1 do
				    ...
				    if xForPlayer.identifier == targetIdentifier then
				    	...
				    	TriggerClientEvent('okokNotify:Alert', xPlayers[i], "BANK", "Vous avez reçu "..amount.."$ de la part "..xPlayer.getName(), 5000, 'success')
				    end
				end
				TriggerEvent('okokBanking:AddTransferTransaction', amount, xTarget, source)
				TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
				TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous avez transférer "..amount.."$ to "..xTarget.getName(), 5000, 'success')
			elseif xTarget == nil then
				...
				TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
				TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous avez tranférer "..amount.."$ to "..targetName, 5000, 'success')
			...
			TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous n avez pas cet argent en banque", 5000, 'error')
		end
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Impossible de s envoyer de l argent à sois même", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous avez déposité "..amount.."$ to "..societyName, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous n avez pas l argent sur vous", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	MySQL.Async.fetchAll('SELECT * de la part okokbanking_societies WHERE society = @society', {
		['@society'] = society
	}, function(result)
		db = result[1]
		hasChecked = true
	...
	if amount <= db.value then
		if db.is_withdrawing == 1 then
			TriggerClientEvent('okokNotify:Alert', _source, "BANK", "Quelqu'un est deja entrain de retiré", 5000, 'error')
		else
		...
		xPlayer.addMoney(amount)
			...
			TriggerClientEvent('okokNotify:Alert', _source, "BANK", "You have withdrawn "..amount.."$ de la part "..societyName, 5000, 'success')
		end
	else
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", "Ton society a pas autant d'argent dans le compte de banque", 5000, 'error')
	end
...
AddEventHandler("okokBanking:TransferMoneyToSociety", function(amount, ibanNumber, societyName, society)
	...
	local playerMoney = xPlayer.getAccount('bank').money
		...
		if amount <= playerMoney then
			...
			TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
			TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous avez tranférer "..amount.."$ to "..societyName, 5000, 'success')
		else
			TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous n avez pas cet argent en banque", 5000, 'error')
		end
end)
...
AddEventHandler("okokBanking:TransferMoneyToSocietyFromSociety", function(amount, ibanNumber, societyNameTarget, societyTarget, society, societyName, societyMoney)
	...
	if amount <= societyMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous avez tranférer "..amount.."$ to "..societyNameTarget, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Votre société ne détient pas cet argent en banque", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:TransferMoneyToPlayerFromSociety", function(amount, ibanNumber, targetIdentifier, acc, targetName, society, societyName, societyMoney, toMyself)
	...
	if amount <= societyMoney then
		...
		elseif xTarget == nil then
			...
			TriggerEvent('okokBanking:AddTransferTransactionFromSocietyToP', amount, society, societyName, targetIdentifier, targetName)
			TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
			TriggerClientEvent('okokNotify:Alert', source, "BANK", "Vous avez transférer "..amount.."$ a "..targetName, 5000, 'success')
		...
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Votre société ne détiens pas cet argent en banque", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if amount <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", "IBAN changé pour "..iban, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", "Vous avez besoin de "..amount.."$ pour changer votre IBAN", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:UpdatePINDB", function(pin, amount)
	...
	if amount <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('okokBanking:updateIbanPinChange', _source)
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", "Pin changé avec succès "..pin, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", "Vous avez besoin de "..amount.."$ pour changer votre PIN", 5000, 'error')
	end
end)